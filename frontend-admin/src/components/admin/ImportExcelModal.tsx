    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          ...
          {parsed && (
            <>
              <Card title={t('preview')} size="small">
                <Table
                  rowKey="_row"
                  dataSource={rows}
                  ...
                  columns={[
                    ...
                    { title: t('price_per_strip'), dataIndex: 'pricePerStrip', render: (v) => v ?? '-' },
                    {
                      ...
                      render: (v, r) =>
                        r._error ? (
                          <span title={r._error} style={{ color: 'red' }}>
                            {v}
                          </span>
                        ) : (
                          v
          ...
          {result && (
            <Card title={t('import_result')} size="small">
              <p>
                {t('created')}: {result.created} | {t('updated')}: {result.updated} | {t('batches')}:{' '}
                {result.batches}
              </p>
              {result.errors?.length ? (
                <p style={{ color: 'red' }}>
